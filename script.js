// Supabase Initialization
 ...
 // ✅ Load Listings (Fixing Undefined Titles)
 ...
 function displayUserOffers(offers) {
   ...
   container.innerHTML = offers.length ? offers.map(offer => `
     <div class="listing-card">
       <p>Offer Price: R${offer.offer_price}</p>
       <p>Status: ${offer.status}</p>
     </div>
   `).join("");
   `).join("") : "<p>You have no offers.</p>";
 }
 ...
 function setupEventListeners() {
   ...
   if (document.getElementById("logout-button")) document.getElementById("logout-button").addEventListener("click", logout);
   if (document.getElementById("dashboard-user-name")) loadDashboard();
